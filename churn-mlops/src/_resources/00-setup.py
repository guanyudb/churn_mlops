...
data_source_schema = dbutils.widgets.get("data_source_schema")
...
if is_advanced_mlops_demo:
  ...
  xp_name = f"dbdemos_advanced_mlops_churn_demo_experiment_{db}"
try:
...
if (db == data_source_schema) and (reset_all_data or not spark.catalog.tableExists(bronze_table_name)):
  import requests
