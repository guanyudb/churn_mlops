...
# DEMO Gate 2 (human-in-the-loop): 04a runs the automated validation and records the
# metric tags only. It does NOT auto-approve or auto-promote. Promotion to @Champion +
# endpoint deployment happens in 04b_challenger_approval ONLY AFTER a human sets the
# model-version tag `Approval_Check` = "Approved" in the Unity Catalog UI.
# (Stock dbdemos auto-approves here — that removes the human gate; do not restore it.)
_all_auto_checks = metric_f1_passed and has_artifacts and has_description and predicts_check and business_metric_passed
# Mark @Challenger so reviewers can find the candidate awaiting approval.
client.set_registered_model_alias(name=model_name, alias="Challenger", version=model_version)
print(
  f"Automated validation passed = {_all_auto_checks}. "
  f"Awaiting HUMAN approval: set model-version tag Approval_Check=Approved on "
  f"{model_name} v{model_version} in Unity Catalog, then re-run this deployment job."
)
