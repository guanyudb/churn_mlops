...
display(spark.table(f"{catalog}.{data_source_schema}.advanced_churn_feature_table"))
...
feature_lookups_n_functions = [
    FeatureLookup(
      table_name=f"{catalog}.{data_source_schema}.advanced_churn_feature_table",
      lookup_key=["customer_id"],
      timestamp_lookup_key="transaction_ts"
    ),
    FeatureFunction(
      udf_name=f"{catalog}.{data_source_schema}.avg_price_increase",
      input_bindings={
...
labels_df = spark.read.table(f"{catalog}.{data_source_schema}.advanced_churn_label_table")
...
def optuna_hpo_fn(n_trials: int, X_train: pd.DataFrame, Y_train: pd.Series, X_test: pd.DataFrame, Y_test: pd.Series, training_set_specs_in, preprocessor_in: ColumnTransformer, experiment_id: str, pos_label_in: str = pos_label, rng_seed_in: int = 2025, run_name:str = "spark-mlflow-tuning", optuna_sampler_in: optuna.samplers.TPESampler = optuna_sampler, optuna_pruner_in: optuna.pruners.BasePruner = None, n_jobs: int = 2) -> optuna.study.study.Study:
    ...
    with mlflow.start_run(run_name=run_name, experiment_id=experiment_id) as run:
        ...
        # Build pip_requirements with pandas pinned for Python 3.12 compat
        import numpy as np, scipy, sklearn as sk
        model_pip_requirements = [
            f"mlflow=={mlflow.__version__}",
            "cloudpickle==3.0.0",
            f"numpy=={np.__version__}",
            "pandas>=2.1,<3",  # Hot fix: pandas 1.5.3 has no wheel for Python 3.12
...
distributed_study = optuna_hpo_fn(
  n_trials=6, # reduced for demo speed
  X_train=X_train,
  X_test=X_test,
  Y_train=Y_train,
  Y_test=Y_test,
